        ...
        await Promise.allSettled(imageNeedQs.map(async (q: any) => {
          ...
          try {
            ...
            if (result.status === 'generated' && result.dataUrl) {
              ...
              if (result.provider) {
                finalProviders[qId] = { provider: result.provider, fallbackReason: result.fallbackReason, requires_labels: result.requires_labels, prompt: result.prompt };
              }
    ...
    try {
      ...
      if (customPrompt) {
        ...
        if (result.status === 'generated' && result.dataUrl) {
          ...
          if (result.provider) {
            setImageProviders(prev => ({ ...prev, [qId]: { provider: result.provider!, fallbackReason: result.fallbackReason, requires_labels: result.requires_labels, prompt: result.prompt } }));
          }
        ...
        await Promise.allSettled(imageNeedQs.map(async (q: any) => {
          try {
            ...
            if (result.status === 'generated' && result.dataUrl) {
              ...
              if (result.provider) {
                finalProviders[q.id] = { provider: result.provider, fallbackReason: result.fallbackReason, requires_labels: result.requires_labels, prompt: result.prompt };
              }
              const providerTag = result.provider === 'openai'
                ? 'openai'
                : result.fallbackReason
  ...
  const refreshImageForQuestion = async (q: any) => {
    ...
    try {
      ...
      if (result.status === 'generated' && result.dataUrl) {
        ...
        if (result.provider) {
          setImageProviders(prev => ({ ...prev, [q.id]: { provider: result.provider!, fallbackReason: result.fallbackReason, requires_labels: result.requires_labels, prompt: result.prompt } }));
        }
        const providerTag = result.provider === 'openai'
          ? 'openai'
          : result.fallbackReason
  ...
  const generateImageForQuestion = async (q: any) => {
    ...
    try {
      ...
      if (result.status === 'generated' && result.dataUrl) {
        ...
        if (result.provider) {
          nextProviders[qId] = { provider: result.provider, fallbackReason: result.fallbackReason, requires_labels: result.requires_labels, prompt: result.prompt };
        }
        bankStore.set({
